...
export default function ForgotPasswordForm() {
  ...
  return (
    <AuthShell tagline={<>Forgotten your<br />password?</>}>
      <div className="px-6 pt-10 pb-12 lg:pt-0 lg:pb-0 lg:px-0">
        ...
        <h1 className="text-[28px] font-bold tracking-tight text-(--ink) leading-[1.15]">Forgot password?</h1>
        <p className="text-[14px] text-(--ink-3) mt-1.5">Enter your email and we&apos;ll send you a reset link.</p>
            ...
            <>
              ...
              <form onSubmit={handleSubmit} className="flex flex-col gap-3.5">
                <AuthField
                  ...
                  disabled={submitting}
                />
                <Button type="submit" variant="primary" size="lg" full disabled={!email || submitting} leading={<Send size={18} weight={2.2} />}>
                  {submitting ? "Sending…" : "Send reset email"}
                </Button>
              </form>
              <div className="mt-4 px-3.5 py-3 rounded-(--r) bg-(--paper-2) border border-(--border) flex gap-2.5 items-start">
                <Alert size={15} style={{ flexShrink: 0, marginTop: 1, color: "var(--ink-3)" }} />
        ...
        <p className="mt-7 text-center text-[13px] text-(--ink-3)">
          Remembered it?{" "}
          <Link href="/login" className="text-(--ink) font-semibold no-underline">Back to sign in</Link>
        </p>
      </div>
    </AuthShell>
  );
